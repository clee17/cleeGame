...
// Sprite_Background
//
// The sprite will automatically scale to the full screen;
...
SceneManager.initialize = function() {
    ...
    //LCJ TODO,这些也都要在Graphics启动的时候就完成；
    this.initAudio();
    this.initInput();
    this.setupErrorHandlers();
};

//LCJ TODO，这一步应该在Graphics的boot中就完成了;
SceneManager.initGraphics = function() {
    var type = Graphics.preferableRendererType(); // this is checked as
    Graphics.setLoadingImage(path+'/img/Loading.png');
    if (type === 'webgl') {
        this.checkWebGL();
    }
};
...
DataManager.loadDatabase = function() {
    ...
    if(!Game_Database)
        return;
    for (var i = 0; i < Game_Database._databaseFiles.length; i++) {
        var name = Game_Database._databaseFiles[i].name;
        var src = Game_Database._databaseFiles[i].src;
...
DataManager.isDatabaseLoaded = function() {
    this.checkError();
    for (var i = 0; i < Game_Database._databaseFiles.length; i++) {
        if (!window[Game_Database._databaseFiles[i].name]) {
            return false;
        }
    }
    return true;
...
Scene_Base.prototype.Scene_Id = 'Scene_Base';
